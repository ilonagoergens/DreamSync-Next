...
import {
  Plus, X, Target, Briefcase, Heart, Users, Sparkles,
  Upload, Download, Move, ChevronsUpDown
} from 'lucide-react';
...
function VisionBoardPage() {
  ...
  const updateItem = async (id, updates) => {
    try {
      ...
      await visionApi.update(id, newItem);
  ...
  const moveItemToSection = (id, targetSection) => {
    ...
    if (sectionElement) {
      const width = 150;
      const height = 150;
      newX = Math.max(0, Math.floor(Math.random() * (sectionElement.clientWidth - width)));
      newY = Math.max(0, Math.floor(Math.random() * (sectionElement.clientHeight - height)));
    }
    ...
    <div className="space-y-6 p-4">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button onClick={downloadVisionBoard} disabled={isDownloading || !items.length}
          ...
          <button onClick={() => setIsAddingItem(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md flex gap-1 items-center">
            <Plus size={18} /> Neues Bild
          </button>
        </div>
      </div>

      <div ref={visionBoardRef} className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {sections.map(sec => (
          <div key={sec.id} className="bg-white rounded-lg p-4 shadow-md">
            ...
            <div ref={(el) => sectionRefs.current[sec.id] = el} className="relative min-h-[300px] bg-gray-50 rounded-lg overflow-hidden">
              {items.filter(i => i.section === sec.id).map(item => (
                <Rnd
                  key={item.id}
                ...
                >
                  <div className="relative w-full h-full group">
                    <img src={item.image_url} alt="" className="w-full h-full object-cover rounded shadow" />
                    <div className="absolute top-1 right-1 flex gap-1 opacity-0 group-hover:opacity-100 transition">
                      <button onClick={() => handleRemoveItem(item.id)} className="bg-white rounded-full p-1 shadow">
                        <X size={14} />
                      </button>
                      <button onClick={() => setSelectedItemId(item.id)} className="bg-white rounded-full p-1 shadow">
                        <Move size={14} />
                      </button>
                      {selectedItemId === item.id && (
                        <div className="absolute right-0 mt-6 bg-white shadow rounded z-50 text-sm">
                          {sections.map(target =>
                            <button key={target.id} onClick={() => moveItemToSection(item.id, target.id)} className="block px-3 py-2 hover:bg-gray-100 w-full text-left">
        ...
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full space-y-4">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => setIsAddingItem(false)}><X size={20} /></button>
            </div>
            <select value={selectedSection} onChange={(e) => setSelectedSection(e.target.value)} className="w-full border px-2 py-1 rounded">
              {sections.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
            </select>
            <input type="text" value={newItemText} onChange={(e) => setNewItemText(e.target.value)} placeholder="Kurze Beschreibung (optional)" className="w-full border px-2 py-1 rounded" />
            <input type="file" accept="image/*" ref={fileInputRef} onChange={handleFileUpload} className="w-full" />
            {uploadedImage && (
              <img src={uploadedImage} alt="Vorschau" className="w-full h-40 object-cover rounded" />
            )}
            <div className="flex justify-end gap-2">
              <button onClick={() => setIsAddingItem(false)} className="px-4 py-2 border rounded">Abbrechen</button>
              <button onClick={handleAddItem} className="px-4 py-2 bg-blue-600 text-white rounded" disabled={!uploadedImage}>Hinzufügen</button>
            </div>
          </div>
        </div>
      )}
